   		...
   		success: function(msg){
   			...
   			for(var k=0;k<t1.length;k++) {
   				...
   				for(var j=0; j<records.length; j++) {
   					...
   					t_elt.push(new Array(createMarker(new GLatLng(point[0],point[1]),"<img src=/image/westpac.gif><br>"+'<b>'+elt[1]+'</b><br>'+elt[2]+'<br>'+elt[5],icon),elt[1]));
   				}
...
if( document.images ) { //check that changing images is supported
  var ClickVariableImg = new Image();
  ClickVariableImg.src = "/image/marker/yellow-dot.png";
  //var ClickVariableImg = new Image();
  //ClickVariableImg.src = "/image/marker/red-dot.png";
}

function action_business_typemike(type,onoff){
	for(var k=0;k<markTab.length;k++) {
		if (markTab[k][0]==type) {
			for(var i=0;i< markTab[k][1].length;i++) {
				if (onoff=="/image/marker/blue-dot.png") {
					map.addOverlay(markTab[k][1][i][0]);
				}
				else
					map.removeOverlay(markTab[k][1][i][0]);
...
<div id="body">
	<div id="body_content">
		<div id="control" style="float:left;width:200px;height:478px;padding-left:10px;padding-top:10px;border:1px solid black;margin-right:2px;text-align:left;background-color:#f9f9f9">
			<?php 
			$t=getTypes();
			
			foreach($t as $k=>$v) {?>
			<div style="height:20;">
				<img src="image/marker/blue-dot.png" name="MarkerType" height="16" width="16" alt="" border="0" style="cursor:pointer" onclick="action_business_type(<?php echo $k?>, true)"> <?php echo $v?>
							</div>
			<?php } ?>
		</div>
